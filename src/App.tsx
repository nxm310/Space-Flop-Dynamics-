...
export function App() {
  ...
  const navTabs = [
    ...
    { id: 'inventory', label: 'Stock Raffiné', icon: <Boxes className="w-4 h-4" /> },
    { id: 'blueprints', label: 'Blueprints & Atelier', icon: <Scroll className="w-4 h-4" /> },
    { id: 'orders', label: 'Carnet Commandes', icon: <ClipboardList className="w-4 h-4" />, badge: orders.filter(o => o.status !== 'completed' && o.status !== 'cancelled').length > 0 ? String(orders.filter(o => o.status !== 'completed' && o.status !== 'cancelled').length) : undefined },
    { id: 'importExport', label: 'Import / Export', icon: <FileSpreadsheet className="w-4 h-4" /> }
  ];

  return (
    <div className="min-h-screen bg-[#090d16] text-slate-100 sc-grid-bg flex flex-col font-sans">
      {/* HUD Top Header */}
      <header className="sticky top-0 z-40 bg-sc-panel/90 backdrop-blur-xl border-b border-sc-border shadow-lg shadow-black/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          >
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-cyan-600 to-sc-cyan flex items-center justify-center text-slate-950 font-bold shadow-neon-cyan group-hover:scale-105 transition-transform">
              <Pickaxe className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="font-bold font-sans tracking-wider text-base text-slate-100 uppercase group-hover:text-sc-cyan transition-colors">
                  Star Citizen
                </span>
                <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-sc-cyan/20 border border-sc-cyan/40 text-sc-cyan font-bold uppercase">
                  Mining & Refinery
                </span>
              </div>
          ...
          <div className="hidden md:flex items-center gap-6 text-xs font-mono">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-100">{rawCargo.reduce((a, c) => a + c.quantitySCU, 0).toFixed(1)} SCU</span>
            </div>

            <div className="flex items-center gap-2">
              <Boxes className="w-3.5 h-3.5 text-emerald-400" />
              <span className="text-slate-400">Raffiné :</span>
              <span className="font-bold text-emerald-400">{refinedStock.reduce((a, s) => a + s.quantitySCU, 0).toFixed(1)} SCU</span>
            </div>

            <div className="flex items-center gap-2">
              <ClipboardList className="w-3.5 h-3.5 text-amber-400" />
              <span className="text-slate-400">Commandes :</span>
              <span className="font-bold text-amber-400">{orders.filter(o => o.status !== 'completed').length}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="hidden sm:flex px-3 py-1.5 rounded-lg bg-sc-cyan/15 hover:bg-sc-cyan text-sc-cyan hover:text-slate-950 border border-sc-cyan/40 text-xs font-mono font-bold uppercase tracking-wider items-center gap-1.5 transition-all duration-200"
            >
            ...
            <button
              ...
              title={settings.soundEnabled ? 'Désactiver les sons' : 'Activer les sons'}
            >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <nav className="flex space-x-1 overflow-x-auto py-1 custom-scrollbar" aria-label="Tabs">
            {navTabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-lg text-xs font-mono uppercase tracking-wider flex items-center gap-2 transition-all shrink-0 ${
                    isActive
                      ? 'bg-sc-card text-sc-cyan border border-sc-cyan/50 shadow-sm font-bold'
                ...
                >
                  ...
                  <span>{tab.label}</span>
                  {tab.badge && (
                    <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-sc-cyan/20 text-sc-cyan border border-sc-cyan/40">
                      {tab.badge}
                    </span>
                  )}
                </button>
